...
| B16 | Historical market data collection for universe calibration | ❌ BLOCKER (for parameter calibration only) | Controller + Claude | Repo currently has zero historical bars/data. Needed to run the calibration process in `universe-selection-analysis.md §6` and move any §3 parameter from PROPOSED to APPROVED (evidence bar defined in §6.5). Does not block APPLY of the rest of the system on its own, but per Controller instruction no numeric universe parameter should be approved before this calibration runs. |

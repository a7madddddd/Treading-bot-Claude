...
| B16 | Historical market data collection for universe calibration | 🟨 IN PROGRESS — Phase 2 APPROVED (research/design + data-acquisition scope only) | Controller + Claude | Methodology in `docs/trading/historical-data-calibration-plan.md`. Root-source recommendation (Norgate Data) and 13-step execution plan in `docs/trading/root-data-source-recommendation.md`. No data purchased/downloaded yet; no code written. Pending Controller decisions: confirm/revise Norgate choice, authorize its $630/yr cost, decide the tick-quote secondary source, confirm Windows VM provisioning. Phase 3 (live implementation) remains NOT approved. Does not block APPLY of the rest of the system on its own, but per Controller instruction no numeric universe parameter should be approved before calibration runs (evidence bar in `historical-data-calibration-plan.md §13`). |

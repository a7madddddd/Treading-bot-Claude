...
| B16 | Historical market data collection for universe calibration | 🟨 IN PROGRESS — Phase 2 APPROVED (research/design + data-acquisition scope only); **Norgate rejected, free-only architecture recommended** | Controller + Claude | Methodology in `docs/trading/historical-data-calibration-plan.md`. Current authoritative root-source recommendation (Stooq + SEC EDGAR + Nasdaq Trader + Yahoo Finance + FRED, all free) and 15-step execution plan in `docs/trading/free-root-data-source-recommendation.md`. Norgate/paid-provider research in `root-data-source-recommendation.md` is REJECTED/superseded. No data purchased/downloaded yet; no code written. Central disclosed limitation: free sources cannot confirm bulk pre-delisting price history for delisted symbols — capped confidence pending an empirically-measured coverage rate. Pending Controller decisions: confirm/revise the free architecture, independently re-verify Stooq/Yahoo terms of use. Phase 3 (live implementation) remains NOT approved. Does not block APPLY of the rest of the system on its own, but per Controller instruction no numeric universe parameter should be approved before calibration runs (evidence bar in `historical-data-calibration-plan.md §13`, extended with the coverage-rate disclosure in the free-architecture document §22). |

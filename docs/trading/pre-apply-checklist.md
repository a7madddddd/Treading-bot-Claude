...
| B16 | Historical market data collection for universe calibration | ❌ BLOCKER — **pilot attempted, could not execute; full acquisition NOT authorized** | Controller + Claude | Controller authorized a small controlled pilot (`docs/trading/data-acquisition-pilot.md`); real `curl` tests against the actual data-serving endpoints of every approved source (Stooq, SEC EDGAR, Nasdaq Trader, FRED, Yahoo) all failed with an identical proxy-level `403 policy denial` — confirmed a systemic network-egress restriction in this environment, not a per-source issue. Only `raw.githubusercontent.com` reachable. Zero real records retrieved; nothing fabricated. Design work completed regardless: 8-case pilot symbol selection (no TSLA), a refined 5-way canonical identity model (company/security/ticker-history/listing-exchange/trading-history, superseding the simpler CIK-only design), point-in-time WE-KNOW/CAN-INFER/CANNOT-KNOW analysis, and 10 PASS/FAIL data-quality gates for future evaluation. Norgate/paid-provider research remains REJECTED/superseded. **Next step is an infrastructure/access decision**, not a data-source decision: needs an execution environment whose network egress can actually reach these domains (Controller's own machine, or a differently-configured/allowlisted session). Phase 3 remains NOT approved. |

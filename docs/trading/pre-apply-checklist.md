...
| B16 | Historical market data collection for universe calibration | 🟨 IN PROGRESS — composite free-source architecture researched (D-0028); verdict C, no free composite solution closes the 2018-2026 OHLCV gap; full acquisition still NOT authorized | Controller + Claude | Controller redirected to GitHub-reachable sources only (`docs/trading/github-native-data-sources.md`). Real, verified findings: `datasets/finance-vix` fully solves market-regime data (PDDL license, current through 2026-09-11). `eliangcs/pystock-data` (cloned and inspected directly, 517MB real data, CC BY-SA 4.0) partially solves bulk OHLCV for **2009–2017 only** (raw + split/dividend-adjusted prices both present and labeled). A real 5-symbol delisted-security test found Family Dollar's price history present, RadioShack/Blockbuster/Dell/Heinz absent (1/5). **Recency pass (D-0027):** four additional single-source candidates (`irachex/open-stock-data`, `hanurd25/stock-data-collector`, `blumenty/stock-data-automation`, `PCnslt/stock-market-data`, `SteelCerberus/us-market-data`) all disqualified — no free single GitHub dataset closes the 2018-2026 gap. **Composite-architecture pass (2026-09-14, `github-native-data-sources.md` §6 / D-0028):** investigated whether combining sources across identity/universe/OHLCV/delisted/regime/corporate-action layers closes the gap. Materially new finding: `fja05680/sp500` gives real, verified, point-in-time daily S&P 500 constituent membership from 1996-01-02 through 2026-08-18 (MIT, actively maintained, delisting timing verified against 3 known events) — the first genuine point-in-time universe source this thread has produced, but scoped to S&P 500 only, not the full dynamic universe D-0026 requires. Identity layer improved via `jadchaar/sec-cik-mapper` and `JerBouma/FinanceDatabase` (both current-snapshot only). Layer D (bulk delisted-securities dataset) disqualified: `EpicSaber/delisted-stocks-list` and `BlackFalconData-org/delisted-stocks-list` both contain only a README, fronting the same paid/metered Apify actor. Layer C (modern OHLCV) still unresolved: `piekstra/market-data` has no committed data, `vijinho/sp500` is index-level only and frozen at 2018. **Access gap, not a disqualification:** three Hugging Face-hosted OHLCV candidates look promising by description but `huggingface.co` is blocked from this environment (same network-policy signature as Stooq/SEC/Yahoo/FRED) — genuinely unverified, flagged as the highest-value next step if reachable from elsewhere. **Conclusion: still no free (single or composite) solution for broad-market OHLCV 2018-2026**; `eliangcs/pystock-data` + `fja05680/sp500` together support only a bounded, disclosed, pre-2018, S&P-500-biased historical control, not primary D-0026 calibration evidence. Norgate/paid-provider research remains REJECTED/superseded. Earlier pilot in `docs/trading/data-acquisition-pilot.md` remains valid. **Pending Controller decision:** verify the Hugging Face candidates from a network this environment cannot reach; or accept the pre-2018/S&P-500-biased historical control as sufficient for a bounded first calibration pass; or pursue direct Stooq/SEC/FRED access from a non-blocked network; or a future explicit paid-provider decision. None of these is recommended or authorized by this finding. Phase 3 remains NOT approved. |

...
| B16 | Historical market data collection for universe calibration | 🟨 IN PROGRESS — working GitHub-native sources found and verified; full acquisition still NOT authorized | Controller + Claude | Controller redirected to GitHub-reachable sources only (`docs/trading/github-native-data-sources.md`). Real, verified findings: `datasets/finance-vix` fully solves market-regime data (PDDL license, current through 2026-09-11). `eliangcs/pystock-data` (cloned and inspected directly, 517MB real data, CC BY-SA 4.0) partially solves bulk OHLCV for **2009–2017 only** (frozen since — a new, disclosed coverage-ceiling tradeoff versus the originally-intended ongoing Stooq access). A real 5-symbol delisted-security test found Family Dollar's price history present, RadioShack/Blockbuster/Dell/Heinz absent (1/5 — reported honestly, not extrapolated to a dataset-wide rate). Reference/identity data (`datasets/s-and-p-500-companies` with CIK, `datasets/nasdaq-listings`) solved for current-snapshot only. Still unresolved: true point-in-time universe membership, true spread/quote data, any OHLCV after March 2017, full-scale delisted-coverage measurement. Norgate/paid-provider research remains REJECTED/superseded. Earlier pilot in `docs/trading/data-acquisition-pilot.md` remains valid (its design, canonical identity model, and quality gates are unaffected by this pivot). **Pending Controller decisions:** accept the March-2017 ceiling vs. keep seeking direct Stooq/SEC/FRED access (e.g. Controller's own machine); authorize extracting `eliangcs/pystock-data`'s full contents (only samples inspected so far); whether to verify two additional untested reference-data candidates. Phase 3 remains NOT approved. |

...
| B15 | Universe subsystem design + Controller approval | 🟨 IN DESIGN | Controller + Claude | First-pass design in `docs/trading/universe-selection-analysis.md`; second-pass evidence-driven parameter validation in `docs/trading/universe-parameter-validation.md` (both PROPOSED / NOT APPROVED). No numeric parameter is approved; most require the backtest experiment in the validation doc §Task 3/F, which needs historical data this repo does not yet have. TSLA is TEST-ONLY (D-0026 §5). No interim `["TSLA"]` fallback. Production engine will not trade until the subsystem can return a valid universe. |
| B16 | Historical market data collection for universe backtesting | ❌ BLOCKER (for parameter validation only) | Controller + Claude | Repo currently has zero historical bars/data. Needed to move any Section C parameter in `universe-parameter-validation.md` from PROPOSED to evidence-based. Does not block APPLY of the rest of the system if Controller accepts PROPOSED parameters as an interim starting point — but no parameter becomes APPROVED without this data or explicit Controller sign-off overriding the "insufficient evidence" findings. |

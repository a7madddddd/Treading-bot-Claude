...
| B16 | Historical market data collection for universe calibration | 🟨 IN PROGRESS — free-GitHub search space exhausted for the 2018-2026 recency requirement with no qualifying candidate found (Conclusion C); full acquisition still NOT authorized | Controller + Claude | Controller redirected to GitHub-reachable sources only (`docs/trading/github-native-data-sources.md`). Real, verified findings: `datasets/finance-vix` fully solves market-regime data (PDDL license, current through 2026-09-11). `eliangcs/pystock-data` (cloned and inspected directly, 517MB real data, CC BY-SA 4.0) partially solves bulk OHLCV for **2009–2017 only**. A real 5-symbol delisted-security test found Family Dollar's price history present, RadioShack/Blockbuster/Dell/Heinz absent (1/5). Reference/identity data (`datasets/s-and-p-500-companies` with CIK, `datasets/nasdaq-listings`) solved for current-snapshot only. **Final recency pass (2026-09-14, `github-native-data-sources.md` §5 / D-0027):** four additional candidates empirically evaluated — `irachex/open-stock-data` disqualified on data-existence grounds (documented GitHub-Releases bars pipeline has zero published Release tags, never actually produced retrievable data); `hanurd25/stock-data-collector` disqualified for lack of historical depth (real data verified, but only ~10 weeks, 2026-07-02 to present); `blumenty/stock-data-automation` disqualified as a rolling 50-day window dependent on the paid Polygon.io API; `PCnslt/stock-market-data` and `SteelCerberus/us-market-data` eliminated on fit/self-disclosed low quality. **Conclusion: no free, GitHub-hosted dataset closes the 2018-2026 recency gap.** `eliangcs/pystock-data` remains the best free root, usable only as a disclosed pre-2018 historical control, not as a modern-regime-capable sole calibration source. Still unresolved: true point-in-time universe membership, true spread/quote data, any broad-market OHLCV after March 2017, full-scale delisted-coverage measurement. Norgate/paid-provider research remains REJECTED/superseded. Earlier pilot in `docs/trading/data-acquisition-pilot.md` remains valid. **Pending Controller decision:** how to close the 2018-2026 recency gap — options include direct Stooq/SEC/FRED access from a non-blocked network (e.g. Controller's own machine), a future explicit paid-provider decision, or continuing to search for a GitHub-native source not yet found; none of these is recommended or authorized by this finding. Phase 3 remains NOT approved. |

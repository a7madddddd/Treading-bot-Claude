...
| B16 | Historical market data collection for universe calibration | ❌ BLOCKER (for parameter calibration only) | Controller + Claude | Repo currently has zero historical bars/data. Full methodology (data requirements, source-sufficiency check against Alpaca's actual documented capabilities, spread-proxy analysis, survivorship-bias design, walk-forward split, regime segmentation, per-parameter calibration plan, control experiment, sensitivity testing, overfitting safeguards, acceptance checklist, phase boundaries) is in `docs/trading/historical-data-calibration-plan.md` — still Phase 1 (planning only). Two confirmed data gaps not solvable by Alpaca alone: point-in-time universe/tradability history, and sector/GICS classification — external sources required, not yet selected. Does not block APPLY of the rest of the system on its own, but per Controller instruction no numeric universe parameter should be approved before this calibration runs (full evidence bar in the plan doc §13). |
